 <?php 
        ...
        if(isset($_POST['backup'])){
           ...
           $command = exec('C:\wamp\bin\mysql\mysql5.6.12\bin\mysqldump --user='.$username.' --password='.$password.' --host='.$hostname.' '.$dbname.' > C:\wamp\www\Evsu_TaclobanRCS\backup\rcstacloban_'.date('Y-m-d-H-i-s').'.sql');
           if(isset($command)){
                echo '<script>
                        $("#logMsg").empty();
                        $("#logMsg").removeClass();

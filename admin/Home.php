 ...
 <div class="row">
      <div class="col-lg-2">
        ...
        <?php include('includes/dbconn.php');
			$s=mysql_query("SELECT year(curDate()) - year(dp) as p FROM tbl_teacher") or die (mysql_error());

			while($row=mysql_fetch_array($s)){
				$p = $row['p'];
				
				if($p==3 ){
				echo'<script>window.alert("Teacher Promotion Available,Check in Teacher info in years of Teaching");</script>';
				
				}}
		?>
        
        <a href="add_teacher.php" data-toggle="tooltip" title="Teacher's Record" data-placement="right"><span class="glyphicon glyphicon-user"></span>&nbsp;Teacher<span class="badge"><?php echo $row['newp'];?></span></a></li>
